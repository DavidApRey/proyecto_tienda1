...
function Page() {
    ...
    const setingDatos = async () => {
        const int_data = await fetch_lista_marcas();
        setData(int_data);
    }
    ...
    const showSwal = (id) => {
        Swal.fire({
            title: "¿Desea eliminar la marca seleccionado?",
            showDenyButton: true,
                ...
                fetch(`https://backendtienda1311.000webhostapp.com/marcaCRUD.php`, {
                    ...
                    .then((responseData) => {
                        ...
                        if (responseData == "200") {
                            Toast.fire({
                                icon: "success",
                                title: "Marca eliminado con exito"
                            });
                        } else {
                            Toast.fire({
                                icon: "error",
                                title: "No se pudo realizar la eliminacion, por favor verificar si existe un articulo con dicho el Marca"
                            });
                        }
                    })
                    .catch((error) => {
    ...
    return (
        <>
            ...
            <div className='flex flex-rows m-6'>
                ...
                <h1 className='text-black text-xl font-bold'>Marcas</h1>
            </div>
            ...
            <div className='m-5'>
                ...
                <Link className="
                            ...
                            bg-150 bg-x-25 hover:scale-102 active:opacity-85 hover:shadow-soft-xs m-2" href="./marca/crear_marca">Añadir</Link>
                    ...
                    renderRowActions={({ row }) => (
                        <Box sx={{ display: 'flex', flexWrap: 'nowrap', gap: '8px' }}>
                            <IconButton color="secondary">
                                <Link href={`/admin/marca/${row.original.id_cons}`}>
                                    <EditIcon />
                                </Link>
                            </IconButton>
