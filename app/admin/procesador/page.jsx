...
import { fetch_lista_procesador } from '@/app/lib/data';
...
function Page() {
    ...
    const setingDatos = async () => {
        const int_data = await fetch_lista_procesador();
        setData(int_data);
    }
    ...
    const showSwal = (id) => {
        Swal.fire({
            title: "¿Desea eliminar el procesador seleccionado?",
            showDenyButton: true,
                ...
                fetch(`http://tiendatecnolo.softdevdsrp.com.co/procesadorCRUD.php`, {
                    ...
                    .then((responseData) => {
                        ...
                        if (responseData == "200") {
                            Toast.fire({
                                icon: "success",
                                title: "Procesador eliminado con exito"
                            });
                        } else {
                            Toast.fire({
                                icon: "error",
                                title: "No se pudo realizar la eliminacion, por favor verificar si existe un articulo con dicho Procesador"
                            });
                        }
                    })
                    .catch((error) => {
    ...
    const columns = [
        ...
        {
            accessorKey: 'num_nucleos',
            header: 'Numero Nucleos',
            size: 150,
        },
        {
            accessorKey: 'estado',
    ...
    return (
        <>
            ...
            <div className='flex flex-rows m-6'>
                ...
                <h1 className='text-black text-xl font-bold'>Procesadores</h1>
            </div>
            ...
            <div className='m-5'>
                ...
                <Link className="
                            ...
                            bg-150 bg-x-25 hover:scale-102 active:opacity-85 hover:shadow-soft-xs m-2" href="./procesador/crear_procesador">Añadir</Link>
                    ...
                    renderRowActions={({ row }) => (
                        <Box sx={{ display: 'flex', flexWrap: 'nowrap', gap: '8px' }}>
                            <IconButton color="secondary">
                                <Link href={`/admin/procesador/${row.original.id_cons}`}>
                                    <EditIcon />
                                </Link>
                            </IconButton>
