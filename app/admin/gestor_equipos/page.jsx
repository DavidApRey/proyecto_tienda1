'use client';
import { IconButton, Box, Typography, Modal } from '@mui/material';
import { MaterialReactTable } from 'material-react-table';
import { MRT_Localization_ES } from 'material-react-table/locales/es';


import {
    Edit as EditIcon,
} from '@mui/icons-material';
import { useState } from 'react';
import { fetch_lista_productos } from '@/app/lib/data';
import Link from 'next/link';

function Page() {

    const [data, setData] = useState([]);

    const setingDatos = async () => {
        const int_data = await fetch_lista_productos();
        setData(int_data);
    }

    const columns = [
        {
            accessorKey: 'id_cons', //access nested data with dot notation
            header: 'Id',
            size: 40,
        },
        {
            accessorKey: 'id_articulo', //access nested data with dot notation
            header: 'ID Articulo',
            size: 150,
        },
        {
            accessorKey: 'nombre_producto', //access nested data with dot notation
            header: 'Nombre Producto',
            size: 150,
        },
        {
            accessorKey: 'precio', //access nested data with dot notation
            header: 'Precio',
            size: 150,
        },
        {
            accessorKey: 'color', //access nested data with dot notation
            header: 'Color',
            size: 150,
        },
        {
            accessorKey: 'modelo',
            header: 'Modelo',
            size: 150,
        },
        {
            accessorKey: 'tamano_pantalla', //normal accessorKey
            header: 'Tañamo Pantalla',
            size: 200,
        },
        {
            accessorKey: 'resolucion_pantalla',
            header: 'Resolucion Pantalla',
            size: 150,
        },
        {
            accessorKey: 'estado',
            header: 'Estado',
            size: 150,
            Cell: ({ renderedCellValue }) => (
                <Box
                    sx={{
                        display: 'flex',
                        alignItems: 'center',
                        gap: '1rem',
                    }}
                >
                    <div className={`center relative inline-block select-none whitespace-nowrap rounded-full ${renderedCellValue == 'Y' ? 'bg-green-500' : 'bg-red-500'} px-3.5 py-1.5 align-baseline font-sans text-xs shadow-xl font-bold uppercase leading-none text-white`}>
                        <span>{renderedCellValue == 'Y' ? 'Activo' : 'Inactivo'}</span>
                    </div>
                </Box>
            ),
        },
    ];

    setingDatos()

    return (
        <>

            <div className='flex flex-rows m-6'>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="text-black w-8 h-8 mx-2">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M11.35 3.836c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m8.9-4.414c.376.023.75.05 1.124.08 1.131.094 1.976 1.057 1.976 2.192V16.5A2.25 2.25 0 0 1 18 18.75h-2.25m-7.5-10.5H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V18.75m-7.5-10.5h6.375c.621 0 1.125.504 1.125 1.125v9.375m-8.25-3 1.5 1.5 3-3.75" />
                </svg>
                <h1 className='text-black text-xl font-bold'>Gestor de Equipos</h1>
            </div>

            <div className='m-5'>

                <Link className="
                            inline-block px-6 py-3 mr-3 font-bold text-center text-white uppercase align-middle transition-all 
                            rounded-lg cursor-pointer bg-sky-800 leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md 
                            bg-150 bg-x-25 hover:scale-102 active:opacity-85 hover:shadow-soft-xs m-2" href="./gestor_equipos/crear_equipo">Añadir</Link>

                <MaterialReactTable
                    columns={columns}
                    data={data}
                    localization={MRT_Localization_ES}
                    enableRowActions
                    renderRowActions={({ row }) => (
                        <Box sx={{ display: 'flex', flexWrap: 'nowrap', gap: '8px' }}>
                            <IconButton color="secondary">
                                <Link href={`/admin/gestor_equipos/${row.original.id_cons}`}>
                                    <EditIcon />
                                </Link>
                            </IconButton>
                        </Box>
                    )}
                />
            </div>

        </>
    )
}

export default Page
